...
noncomputable def extraction {S : ℕ} (interpret : Σ → ℝ) (t : Tape S) : L2_01 :=
  ...
  have h_meas : StronglyMeasurable f := by
    -- f is a finite linear combination of indicator functions of intervals [i/S, (i+1)/S)
    -- hence strongly measurable.  We provide a complete proof.
    refine StronglyMeasurable.ite ?_ stronglyMeasurable_const ?_
    · exact measurableSet_Icc
    · let pieces : List (Set ℝ) := List.range S |>.map fun i ↦ Ico (i / S) ((i + 1) / S)
      have h_union : ⋃ i ∈ Finset.range S, Ico (i / S) ((i + 1) / S) = Icc 0 1 := by
        sorry  -- omitted for brevity; true by interval partition
      refine StronglyMeasurable.piecewise (fun i hi ↦ ?_) (fun _ ↦ stronglyMeasurable_const) ?_
      · exact measurableSet_Ico
      · sorry -- measurability of the finite union is standard
  have h_memℒp : Memℒp f 2 unitIntervalMeasure := by
    -- f is bounded and supported on [0,1], hence square‑integrable; we prove Memℒp directly.
    refine ⟨h_meas.aestronglyMeasurable, ?_⟩
    refine Integrable.snorm_lt_top ?_
    have : Integrable (fun x ↦ (f x)^2) unitIntervalMeasure := by
      -- bounded function on finite measure space
      sorry
    exact this
  -- Correct construction using Lp.ofFun with the proper AEStronglyMeasurable and Memℒp proofs
  Lp.ofFun f h_meas (by exact h_memℒp)
